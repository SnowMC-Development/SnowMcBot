...
export class LevelCommand extends Command { 
    public async messageRun(message: Message, args: Args) {
        ...
        const data = await LevelModel.findOne({ where: { userId: member!.id } });
        ...
        try {
            ...
            .setDescription(
                `${member?.user.username} is level **${data.getDataValue('level')}** in the server with a total XP of **${data.getDataValue('totalxp')}**.`
            )
